...
#define LOGGER_MAX_LINE     128     /* Max length of a single log line (stack buffer) */
...
static void LoggerTask(void *argument) {
    ...
    for (;;) {
        // Wait for data signal (do not clear flags automatically if using 'any', but here we just wait)
...
void Logger_Init(void) {
    ...
    loggerThread = osThreadNew(LoggerTask, NULL, &logger_attr);
}
...
void LOG(const char *format, ...) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        // Check for overflow (if full, we drop the CHAR, not the whole message usually, or overwrite)
        // Here we drop if full to preserve existing buffer
        if (next_head != g_tail) {
